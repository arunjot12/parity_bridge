...
pub struct BlockContentIterator<'a> {
	...
	genesis_hash: substrate2_node_primitives::Hash,
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let signed = self.keyring.sign(
			CheckedExtrinsic {
				signed: Some((
					sender,
					signed_extra(0, runtime::ExistentialDeposit::get() + 1),
				)),
				function: match self.content.block_type {
					BlockType::RandomTransfersKeepAlive =>
						RuntimeCall::Balances(BalancesCall::transfer_keep_alive {
							dest: sp_runtime::MultiAddress::Id(receiver),
							value: runtime::ExistentialDeposit::get() + 1,
						}),
					BlockType::RandomTransfersReaping => {
						RuntimeCall::Balances(BalancesCall::transfer {
							...
							value: 100 * DOLLARS -
								(runtime::ExistentialDeposit::get() - 1),
						})
					},
					BlockType::Noop =>
...
impl BenchKeyring {
	...
	pub fn generate_genesis(&self) -> runtime::GenesisConfig {
		crate::genesis::config_endowed(
			Some(runtime::wasm_binary_unwrap()),
